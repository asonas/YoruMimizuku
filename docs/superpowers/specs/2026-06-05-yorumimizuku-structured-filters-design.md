...
- OR で一部サブクエリが失敗した場合: v1 は **fail-fast**。いずれかのサブクエリの `searchPosts` が失敗したら `loadPage` 全体を throw し、既存の再試行 UI に委ねる。複合カーソルは更新しないので、再試行で同じフェッチをやり直せる。
  - （初期検討では「部分成功」を採用予定だったが、複合カーソルで「未取得 nil」「枯渇 nil」「失敗」を区別できず、初回失敗サブクエリの永久スキップや恒久失敗時の空ページ無限生成というバグを生むため、fail-fast に変更した。）
  - フォローアップページでは、サブクエリの sub-cursor が nil = 枯渇（スキップ）として扱う。複合カーソルがサブクエリ数と不一致なら（編集直後など）先頭ページとして扱う防御を入れる。
...
- サイドバー要約の最終フォーマット。
- 部分失敗時の挙動（§5 は fail-fast を採用）。
- **OR のページ跨ぎ時系列順は厳密でない**: 各サブクエリは独立にページングするため、密なサブクエリが時間的に先行し、後続ページで既出より新しい投稿が現れ得る。1ページ内の `createdAt` 降順・URI 重複排除は保証する。v1 の既知制約。
