...
| client-metadata ホスト | `ason.as`（`https://ason.as/hoshidukiyo/client-metadata.json`、path は実装時確定） |
...
native の OAuth クライアントは `client_id` として HTTPS 上に静的 JSON 1 枚を公開する必要がある。Hoshidukiyo は `ason.as` 上に配置する。
...
- 実装着手時に確定する項目: 公開パス（`/hoshidukiyo/...` で良いか）、`client_name`、必要なら `transition:email` の追加。
...
- bundle id の確定（案: `as.ason.Hoshidukiyo`）
