...
| client-metadata ホスト | `ason.as`（`https://ason.as/night-sky/client-metadata.json`、path は実装時確定） |
...
native の OAuth クライアントは `client_id` として HTTPS 上に静的 JSON 1 枚を公開する必要がある。NightSky は `ason.as` 上に配置する。

```json
{
  "client_id": "https://ason.as/night-sky/client-metadata.json",
  "client_name": "NightSky",
...
- 実装着手時に確定する項目: 公開パス（`/night-sky/...` で良いか）、`client_name`、必要なら `transition:email` の追加。
...
- アプリ名の最終確定（現状 NightSky）と bundle id（案: `as.ason.NightSky`）
