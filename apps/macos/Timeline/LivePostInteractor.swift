...
/// Live `PostInteracting`: builds a `LiveServiceContext` and runs the like / repost
/// create and delete records through `PostService`, persisting any refreshed
/// tokens. The viewer's own DID is the `repo` every record is written to.
struct LivePostInteractor: PostInteracting {
    ...
    func like(uri: String, cid: String) async throws -> String {
        ...
        let result = try await service(context).like(
            ...
            accessToken: context.account.accessToken, refreshToken: context.account.refreshToken,
            repo: context.account.did, subject: StrongRef(uri: uri, cid: cid)
        )
    ...
    private func delete(recordURI: String, collection: String) async throws {
        ...
        let refreshed = try await service(context).deleteRecord(
            pds: context.account.pds, issuer: context.issuer,
            accessToken: context.account.accessToken, refreshToken: context.account.refreshToken,
            repo: context.account.did, collection: collection, rkey: rkey
        )
